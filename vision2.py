...
INTERSECTION_MAP = {
    ...
    (False, False, True,  False): "DEAD-END-RIGHT",
    (True,  False, False, True ): "DEAD-END-LEFT",
...
def detect_shape(frame, cnt):
    ...
    if angle is not None:
        ...
        cv2.putText(frame, f"{angle:.2f} degrees",
                    (center[0] + 10, center[1]),
                    cv2.FONT_HERSHEY_SIMPLEX, 0.6, (0, 255, 255), 2)
    return angle_to_bias(angle)
...
def detect_intersection(mask, frame, roi_y):
    ...
    label = INTERSECTION_MAP.get((left_active, center_active, right_active, sq_active), f"UNKNOWN ({left_active},{center_active},{right_active},{sq_active})")
    ...
    for i, (zname, x_start) in enumerate(zip(zone_names, zone_starts)):
        ...
        cv2.putText(frame, f"{'■' if active else '□'} {zname.upper()}", (x_start + 4, roi_y + 20),
    ...
    cv2.putText(frame, f"INTERSECTION: {label}",
                (10, fh - 12),
                cv2.FONT_HERSHEY_SIMPLEX, 0.6, banner_color, 2)

    return label, left_active, center_active, right_active, sq_active

def ask_direction(intersection_type):
    global pending_direction, input_thread_running
    choices = VALID_CHOICES.get(intersection_type, [])
    prompt  = f"[INPUT] {intersection_type} — choose ({'/'.join(choices)}): "
    while True:
        answer = input(prompt).strip().upper()
        if answer in choices:
            pending_direction    = answer
            input_thread_running = False
            break
        print(f"  Invalid. Options: {choices}")

def trigger_turn(direction, label, now):
    global state, pending_direction, input_thread_running
    global intersection_count, last_intersection_time, current_intersection
    global integral, prev_error, last_error
    intersection_count     = 0
...
def main():
    ...
    try:
        while True:
            ...
            cnt_center  = largest_contour(center_mask)
            line_found  = False

            if cnt_center is not None and cv2.contourArea(cnt_center) > MIN_CONTOUR_AREA:
                M = cv2.moments(cnt_center)
                if M["m00"] != 0:
                    cx             = int(M["m10"] / M["m00"]) + third
                    cy             = int(M["m01"] / M["m00"])
                    cx_global      = cx + bias
                    cy_global      = cy + roi_y
                    line_found     = True
            ...
            if state == STATE_FOLLOWING:
                needs_prompt = intersection_label in VALID_CHOICES
                needs_auto   = intersection_label in AUTO_EXECUTE
                actionable   = needs_prompt or needs_auto
                ...
                if intersection_count >= INTERSECTION_CONFIRM_FRAMES and cooldown_ok:
                    if needs_auto:
                        direction = AUTO_EXECUTE[intersection_label]
                        print(f"[AUTO] {intersection_label} → {direction}")
                        trigger_turn(direction, intersection_label, now)

                    elif needs_prompt:
                        intersection_count     = 0
                        current_intersection   = intersection_label
                        last_intersection_time = now
                        state                  = STATE_WAITING
                        stop()
                        ...
                        pending_direction      = None
                        threading.Thread(
                            target=ask_direction,
                            args=(current_intersection,),
                            daemon=True
                        ).start()
                    ...
                    if elapsed < SEARCH_TIMEOUT:
                        if last_error > 0:
                            ...
                            cv2.putText(frame, "SEARCHING RIGHT", (10, 30), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 165, 255), 2)
                        else:
                            send_command(-SEARCH_SPEED, SEARCH_SPEED)
                            cv2.putText(frame, "SEARCHING LEFT",  (10, 30), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 165, 255), 2)
                        cv2.putText(frame, f"Timeout: {SEARCH_TIMEOUT - elapsed:.1f}s", (10, 60), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (0, 165, 255), 2)
                    else:
                        stop()
                        cv2.putText(frame, "LINE LOST - STOP", (10, 30), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 0, 255), 2)

            elif state == STATE_WAITING:
                stop()
                cv2.putText(frame, f"WAITING: {current_intersection}", (10, 30), cv2.FONT_HERSHEY_SIMPLEX, 0.65, (0, 100, 255), 2)
                ...
                cv2.putText(frame, f"TURNING {pending_direction}",
                ...
                if pending_direction == "R":
                    ...
                    side_cnt      = largest_contour(side_zone)
                    if side_cnt is not None and cv2.contourArea(side_cnt) > MIN_CONTOUR_AREA:
                        M = cv2.moments(side_cnt)
                        if M["m00"] != 0:
                            raw_error = int(M["m10"] / M["m00"]) + 2 * third - center_x
                    ...
                    if side_cnt is not None and cv2.contourArea(side_cnt) > MIN_CONTOUR_AREA:
                        M = cv2.moments(side_cnt)
                        if M["m00"] != 0:
                            raw_error = int(M["m10"] / M["m00"]) - center_x
                    turn_done = center_has_line and not side_has_line

                else:
                    center_cnt = largest_contour(center_zone)
                    if center_cnt is not None and cv2.contourArea(center_cnt) > MIN_CONTOUR_AREA:
                        M = cv2.moments(center_cnt)
                        if M["m00"] != 0:
                            raw_error = int(M["m10"] / M["m00"]) + third - center_x
                    turn_done = center_has_line

                derror    =  raw_error - prev_error
                integral  += raw_error
                integral   =  clamp(integral, -ki_max, ki_max)
                turn       =  (kp_turn * raw_error) + (kd_turn * derror) + (ki_turn * integral)
                ...
                if turn_done:
                    print(f"[INFO] Turn complete — resuming follow")
                    integral               = 0
                    prev_error             = 0
